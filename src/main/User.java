...
public class User {
    ...
    public void incProgress(int level) {
        currentRank.incProgress(level);
        progress = currentRank.getProgress();
...
class Rank {
    ...
    private int calculateProgress(int level) {
        ...
        if ((level == -1 && rank == 1)) {
            ...
            if (rank < 0) {
                diff = Math.abs(rank) + level;
            } else {
                diff = -rank + level;
            }
        } else if (level > 0) {
            if (rank < 0) {
                diff = Math.abs(rank) + level -1;
            } else {
                diff = level - rank;
            }
        }
        ...
        if (diff == 0) {
            ...
            newProgress = progress + 10 * diff * diff;
        } else {
            return progress;
        }

        if (newProgress >= maxProgress) {
            int count = newProgress / maxProgress;
            ...
            newProgress = newProgress - count * 100;
        }
